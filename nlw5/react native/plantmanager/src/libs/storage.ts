...
export async function loadPlant(): Promise<PlantProps[]> {
  try {
    ...
    const plantsSorted = Object.keys(plants).map(plant => {
      ...
      Math.floor(
        new Date(a.dateTimeNotification).getTime() / 100 - Math.floor(new Date(b.dateTimeNotification).getTime() / 100)
      )
    )
...
export async function removePlant(id: string): Promise<void> {
  ...
  const data_aux =  await AsyncStorage.getItem('@plantmanager:plants')
}